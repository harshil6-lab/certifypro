...
import { AdobeTemplateGallery } from "@/components/templates/AdobeTemplateGallery";

type BuiltinTemplateGalleryProps = {
  onSelectTemplate: (template: CertificateTemplateMeta) => void;
  selectedId?: string;
  showAdobeOption?: boolean;
};
...
export function BuiltinTemplateGallery({ onSelectTemplate, selectedId, showAdobeOption = true }: BuiltinTemplateGalleryProps) {
  // showAdobeOption defaults to true (prop is optional).
  // Note: We must keep existing behavior when Adobe option is hidden.
  const [activeSource, setActiveSource] = useState<"certifypro" | "adobe">("certifypro");
  ...
  useEffect(() => {
    if (!showAdobeOption && activeSource === "adobe") {
      setActiveSource("certifypro");
    }
  }, [activeSource, showAdobeOption]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <Button
            type="button"
            size="sm"
            variant={activeSource === "certifypro" ? "default" : "outline"}
            className={activeSource === "certifypro" ? "gold-gradient text-accent-foreground" : ""}
            onClick={() => setActiveSource("certifypro")}
          >
            CertifyPro Templates
          </Button>
          {showAdobeOption ? (
            <Button
              type="button"
              size="sm"
              variant={activeSource === "adobe" ? "default" : "outline"}
              className={activeSource === "adobe" ? "gold-gradient text-accent-foreground" : ""}
              onClick={() => setActiveSource("adobe")}
            >
              Adobe Express
            </Button>
          ) : null}
      ...
      {activeSource === "adobe" && showAdobeOption ? (
        <AdobeTemplateGallery
